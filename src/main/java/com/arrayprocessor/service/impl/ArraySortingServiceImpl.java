...
public class ArraySortingServiceImpl implements ArraySortingService {
    ...
    @Override
    public void sort(double[] array, SortingAlgorithm algorithm) throws ArrayProcessingException {
        if (array == null) {
            throw new ArrayProcessingException("Array must not be null");
        }
        if (algorithm == SortingAlgorithm.BUBBLE) {
            bubbleSortDouble(array);
        } else {
            insertionSortDouble(array);
        }
    }
    ...
    private void bubbleSortDouble(double[] array) {
        int length = array.length;
        for (int i = 0; i < length - 1; i++) {
            for (int j = 0; j < length - i - 1; j++) {
                if (array[j] > array[j + 1]) {
    ...
    private void insertionSortDouble(double[] array) {
        int length = array.length;
        for (int i = 1; i < length; i++) {
            double key = array[i];
            int j = i - 1;
